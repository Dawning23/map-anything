...
torchrun --nproc_per_node ${MLP_WORKER_GPU} --nnodes ${MLP_WORKER_NUM} --node_rank ${MLP_ROLE_INDEX} \
    --master_addr=$MLP_WORKER_0_HOST \
    --master_port=$MLP_WORKER_0_PORT \
    scripts/train.py \
    machine=huoshan \
    dataset=tartan_ground_fisheye \
    dataset.num_workers=128 \
    ...
    train_params.max_num_of_imgs_per_gpu=72 \
    hydra.run.dir='${root_experiments_dir}/mapanything/training/mapa_fisheye_04201413'
